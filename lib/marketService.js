 ...
 * Reads directly from upstream Yahoo Finance endpoints (no seed or fallback data).
 */
...
/**
 * Fetch raw chart data from Yahoo Finance upstream
 */
async function fetchYahooChart(symbol, range = '1mo', interval = '1d') {
  ...
  if (!res.ok) {
    const error = new Error(`Failed to fetch ${symbol} from Yahoo Finance: upstream returned status ${res.status}.`);
  ...
  if (!result || !result.meta) {
    const error = new Error(`Failed to parse chart result for ${symbol} from Yahoo Finance: upstream returned status 502.`);
    error.status = 502;
    throw error;
  }
...
export async function getStockQuote(symbol) {
  ...
  const chartResult = await fetchYahooChart(sym, '1d', '1d');
  const meta = chartResult.meta;
...
export async function getStockHistory(symbol, timeframe = '1M') {
  ...
  const chartResult = await fetchYahooChart(sym, range, interval);
...
export async function getMaangPrices(symbol) {
  ...
  const chartResult = await fetchYahooChart(sym, '1mo', '1d');
...
export async function getMaangIndicators(symbol, indicator = 'ALL') {
  ...
  const chartResult = await fetchYahooChart(sym, '1y', '1d');
  const { bars } = parseBars(chartResult);
...
export async function getPortfolioAllocation({ symbol1, symbol2, symbol3, symbol4, symbol5 }) {
  ...
  const tickerStats = await Promise.all(
    uniqueSymbols.map(async (sym) => {
      ...
      const { meta, bars } = parseBars(chartResult);
      if (bars.length < 5) {
        const error = new Error(`Failed to compute volatility for ${sym}: insufficient data from Yahoo Finance.`);
        error.status = 502;
        throw error;
      }
